...
def combine_csv_files(directory):
    try:
        ...
        csv_files = [file for file in os.listdir(directory) if file.startswith('navio_') and file.endswith('.csv')]
        ...
        if dfs:
            try:
                ...
                output_file = os.path.join(directory,'ships_status_log.csv')
                df_final.to_csv(output_file, index=False)
                print(f'Dados combinados foram salvos em {output_file}')

                # Adicionar função para adicionar ID único no log
